...
- `edge/detector.py`：封装 YOLO 检测器。默认从根目录 `public/` 查找 `.pt/.onnx/.engine` 模型；模型缺失或加载失败直接报错，不做模拟降级。
...
- `edge/runner.py`：命令行入口，完成摄像头采集、本地推理、调度和上报流程。
...
- 真实摄像头：`edge/runner.py` 默认连续采集本机摄像头，`--once` 可只处理一帧。
