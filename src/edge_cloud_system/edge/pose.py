...
class PoseAnalyzer:
    """Rule-based pose action classifier for edge-side fallback."""
    ...
    def _classify_rule(
        ...
        *,
        ...
        torso_ratio: float,
    ) -> tuple[PoseAction, float, str, str, list[str]]:
        width, height = frame_size
        ...
        if shoulder_y is not None and hip_y is not None and knee_y is not None:
            ...
            if torso_down > 0.20 and leg_drop > 0.18:
                ...
                return (
                    PoseAction.STANDING,
                    confidence,
                    "standing_posture_rule",
                    "肩部、髋部和膝部存在清晰的上下层次，符合站立姿态。",
                    evidence,
                )
            ...
            if torso_down > 0.12 and normalized_gap(hip_y, knee_y) is not None and normalized_gap(hip_y, knee_y) < 0.10:
                ...
                return (
                    PoseAction.CROUCHING,
                    confidence,
                    "crouching_posture_rule",
                    "髋部和膝部贴近且整体重心下移，符合蹲姿特征。",
                    evidence,
                )
        ...
        if shoulder_y is not None and hip_y is not None:
            if shoulder_y < hip_y:
                ...
                return (
                    PoseAction.STANDING,
                    min(0.84, confidence),
                    "fallback_upright_rule",
                    "身体呈现基本竖直关系，按照默认规则判为站立。",
                    evidence,
                )
